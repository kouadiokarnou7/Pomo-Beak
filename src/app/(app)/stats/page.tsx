...
export default function StatsPage() {
  ...
  const [heatmapPeriod, setHeatmapPeriod] = useState<30 | 90 | 365>(90);
  ...
  const contributionData = useMemo(() => calculateHeatmapData(tasks, heatmapPeriod), [tasks, heatmapPeriod]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-7 glass-panel rounded-xl p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-1 bg-surface-container/50 rounded-lg p-1 border border-border-glass">
              {[
                { value: 30, label: "30j" },
                { value: 90, label: "90j" },
                ...
                <button
                  key={opt.value}
                  onClick={() => setHeatmapPeriod(opt.value as 30 | 90 | 365)}
                  className={`text-[10px] font-bold px-3 py-1 rounded-md transition-all ${heatmapPeriod === opt.value ? 'bg-primary text-background shadow-sm' : 'text-on-surface-variant hover:text-on-surface hover:bg-surface-glass'}`}
                >
                  {opt.label}
                </button>
              ))}
          ...
          <div className="flex-1 flex flex-col justify-center w-full">
            {(() => {
              // Taille harmonieuse et élégante pour toutes les périodes (évite la déformation verticale)
              const cellSize = heatmapPeriod <= 90 ? 'w-5 h-5' : 'w-3.5 h-3.5';
              const gapSize = heatmapPeriod <= 90 ? 'gap-1.5' : 'gap-1';
              const cellRound = 'rounded-md';
              const colWidth = heatmapPeriod <= 90 ? 'w-5' : 'w-3.5';
              const labelSpacing = heatmapPeriod <= 90 ? 26 : 18; // 20px + 6px gap ou 14px + 4px gap
              const legendSize = 'w-4 h-4';
                ...
                <>
                  <div className="w-full overflow-x-auto pb-4">
                    <div className="flex flex-col">
                      ...
                      <div className="flex text-[10px] text-on-surface-variant mb-1 relative h-4 min-w-full">
                        {monthLabels.map((ml) => (
                          <span 
                            key={ml.index} 
                            className="absolute"
                            style={{ left: `${ml.index * labelSpacing}px` }}
                          >
                            {ml.label}
                          </span>
                        ))}
                      ...
                      {/* Grille */}
                      <div className={`flex ${gapSize} min-w-max`}>
                        {weeks.map((week, weekIndex) => (
                          <div key={weekIndex} className={`flex flex-col ${gapSize} ${colWidth}`}>
                            {week.map((day) => {
                              ...
                              return (
                                <div 
                                  key={day.date}
                                  className={`${cellSize} ${cellRound} ${bgColor} hover:ring-2 hover:ring-on-surface transition-all cursor-pointer`}
                  ...
                  {/* Légende */}
                  <div className="flex items-center gap-2 mt-4 text-[11px] text-on-surface-variant w-full justify-end px-4">
                     <span>Moins</span>
                     <div className={`${legendSize} rounded-md bg-surface-container/30`}></div>
                     <div className={`${legendSize} rounded-md bg-primary/30`}></div>
                     <div className={`${legendSize} rounded-md bg-primary/50`}></div>
                     <div className={`${legendSize} rounded-md bg-primary/80`}></div>
                     <div className={`${legendSize} rounded-md bg-primary`}></div>
                     <span>Plus</span>
                  </div>
                </>
              );
